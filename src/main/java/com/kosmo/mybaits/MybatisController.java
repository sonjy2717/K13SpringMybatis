...
public class MybatisController {
	...
	public String list(Model model, HttpServletRequest req) {
		...
		//페이지 처리를 위한 설정값
		int pageSize = 4; //한 페이지당 출력할 게시물의 개수
		...
		//현재 페이지 번호 설정
		/*
		방명록URL?nowPage=		-> 이 경우 페이지 번호는 빈값
		...
		/*
		서비스 역할 인터페이스의 추상메서드를 호출하면 mapper가 동작됨
	...
	public String listSearch(Model model, HttpServletRequest req) {
		...
		String pagingImg = PagingUtil.pagingImg(totalRecordCount, pageSize,
				blockPage, nowPage, req.getContextPath()+"/mybatis/listSearch.do?");
	...
	public String write(Model model, HttpSession session, HttpServletRequest req) {
		...
		if (session.getAttribute("siteUserInfo") == null) {
			/*
			현재상태는 글쓰기를 위해 버튼을 클릭했으므로 만약 로그인이
			완료되었다면 글쓰기 페이지로 이동하는 것이 좋다.
			따라서 backUrl이라는 파라미터에 쓰기페이지의 View경로를 붙여서
			리다이렉트 시킨다.
			 */
	...
	public String writeAction(Model model, HttpServletRequest req, HttpSession session) {
		...
		/*
		글쓰기 페이지에 오래동안 머물러 세션이 끊어지는 경우가 있으므로
		...
		//insert문을 실행시 입력에 성공한 행의 개수가 정수형으로 반환된다.
	...
	//수정페이지에서 전송된 폼값은 커맨드 객체를 통해 한꺼번에 받는다.
